# HelpDesk — Documentação do Sistema (v0.0.6)
...
| **v0.0.6 — validações da equipe** | **Código ÚNICO de acesso por grupo**: todo mundo entra como **solicitante** (escolhendo a cidade — seleção das cidades cadastradas) e os **técnicos do grupo** promovem para dev/suporte no perfil do membro; convites idem ("Convidar", entra como solicitante). **Aba "Visão geral" na Equipe**: números do grupo, **técnicos online** (presença por atividade, janela de 5 min), **categorias × membros** e carga ativa por técnico. **Painel mais personalizável**: novos widgets — por sistema, por cidade, tempo médio de resolução (geral e por urgência), análises pendentes e atendimentos dos últimos 7 dias (suporte). |
